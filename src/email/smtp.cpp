...
SmtpClient::SmtpClient(Socket::ptr sock)
    :stone::SocketStream(sock) {
}

SmtpClient::ptr SmtpClient::Create(const std::string& host, uint32_t port, bool ssl) {
    stone::IPAddress::ptr addr = stone::Address::LookupAnyIPAddress(host);
    if(!addr) {
        STONE_LOG_ERROR(g_logger) << "invalid smtp server: " << host << ":" << port
            << " ssl=" << ssl;
        return nullptr;
    }
    addr->setPort(port);
    Socket::ptr sock;
    if(ssl) {
        //sock = stone::SSLSocket::CreateTCP(addr);
    } else {
        sock = stone::Socket::CreateTCP(addr);
    }
    if(!sock->connect(addr)) {
        STONE_LOG_ERROR(g_logger) << "connect smtp server: " << host << ":" << port
            << " ssl=" << ssl << " fail";
        return nullptr;
    }
    std::string buf;
    ...
    buf.resize(len);
    if(stone::TypeUtil::Atoi(buf) != 220) {
        return nullptr;
    }
    rt->m_host = host;
    return rt;
...
SmtpResult::ptr SmtpClient::doCmd(const std::string& cmd, bool debug) {
    ...
    int code = stone::TypeUtil::Atoi(buf);
    if(code >= 400) {
...
#define DO_CMD() \
    ...
    if(!m_authed && !email->getFromEMailAddress().empty()) {
        ...
        cmd = stone::base64encode(email->getFromEMailAddress().substr(0, pos)) + "\r\n";
        DO_CMD();
        cmd = stone::base64encode(email->getFromEMailPasswd()) + "\r\n";
        DO_CMD();
...
#define XX(fun) \
    ...
    std::string boundary;
    if(!entitys.empty()) {
        boundary = stone::random_string(16);
