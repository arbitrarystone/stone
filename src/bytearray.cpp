...
bool ByteArray::isLittleEndian() const {
    return m_endian == STONE_LITTLE_ENDIAN;
}

void ByteArray::setLittleEndian(bool val) {
    if(val) {
        m_endian = STONE_LITTLE_ENDIAN;
    } else {
        m_endian = STONE_BIG_ENDIAN;
    }
}
...
bool ByteArray::writeToFile(const std::string& name) const {
    ...
    if(!ofs) {
        STONE_LOG_ERROR(g_logger) << "writeToFile name=" << name
...
bool ByteArray::readFromFile(const std::string& name) {
    ...
    if(!ifs) {
        STONE_LOG_ERROR(g_logger) << "readFromFile name=" << name
            << " error, errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
