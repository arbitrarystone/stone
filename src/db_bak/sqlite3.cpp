...
static stone::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_sqlite3_dbs
    = stone::Config::Lookup("sqlite3.dbs", std::map<std::string, std::map<std::string, std::string> >()
            , "sqlite3 dbs");
...
int SQLite3Stmt::bindTime(int idx, const time_t& value) {
    return bind(idx, stone::Time2Str(value));
}
...
time_t SQLite3Data::getTime(int idx) {
    ...
    return stone::Str2Time(str.c_str());
}
...
SQLite3Transaction::~SQLite3Transaction() {
    if(m_status == 1) {
        ...
        if(m_status == 1) {
            STONE_LOG_ERROR(g_logger) << m_db << " auto_commit=" << m_autoCommit << " fail";
        }
    }
}
...
SQLite3::ptr SQLite3Manager::get(const std::string& name) {
    ...
    lock.unlock();
    std::string path = stone::GetParamValue<std::string>(args, "path");
    if(path.empty()) {
        STONE_LOG_ERROR(g_logger) << "open db name=" << name << " path is null";
        return nullptr;
    }

    if(path.find(":") == std::string::npos) {
        path = stone::EnvMgr::GetInstance()->getAbsoluteWorkPath(path);
    }

    sqlite3* db;
    if(sqlite3_open_v2(path.c_str(), &db, SQLite3::CREATE | SQLite3::READWRITE, nullptr)) {
        STONE_LOG_ERROR(g_logger) << "open db name=" << name << " path=" << path << " fail";
        return nullptr;
    }

    SQLite3* rt = new SQLite3(db);
    std::string sql = stone::GetParamValue<std::string>(args, "sql");
    if(!sql.empty()) {
        if(rt->execute(sql)) {
            STONE_LOG_ERROR(g_logger) << "execute sql=" << sql
...
int SQLite3Manager::execute(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        STONE_LOG_ERROR(g_logger) << "SQLite3Manager::execute, get(" << name
            << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr SQLite3Manager::query(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        STONE_LOG_ERROR(g_logger) << "SQLite3Manager::query, get(" << name
            << ") fail, format=" << format;
        return nullptr;
    }
...
SQLite3Transaction::ptr SQLite3Manager::openTransaction(const std::string& name, bool auto_commit) {
    auto conn = get(name);
    if(!conn) {
        STONE_LOG_ERROR(g_logger) << "SQLite3Manager::openTransaction, get(" << name
            << ") fail";
        return nullptr;
    }
