...
static stone::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_thread_info_set
...
void FoxThread::read_cb(evutil_socket_t sock, short which, void* args) {
    ...
    if(recv(sock, cmd, sizeof(cmd), 0) > 0) {
        ...
        for(auto it = callbacks.begin();
                it != callbacks.end(); ++it) {
            if(*it) {
                //STONE_ASSERT(thread == GetThis());
                try {
                    (*it)();
                } catch (std::exception& ex) {
                    STONE_LOG_ERROR(g_logger) << "exception:" << ex.what();
                } catch (const char* c) {
                    STONE_LOG_ERROR(g_logger) << "exception:" << c;
                } catch (...) {
                    STONE_LOG_ERROR(g_logger) << "uncatch exception";
                }
            } else {
        ...
        stone::Atomic::addFetch(thread->m_total, callbacks.size());
...
FoxThread::FoxThread(const std::string& name, struct event_base* base)
    ...
    if(evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, fds) == -1) {
        //STONE_LOG_ERROR(g_logger) << "FoxThread init error";
...
void FoxThread::start() {
    if(m_thread) {
        //STONE_LOG_ERROR(g_logger) << "FoxThread is running";
...
bool FoxThreadPool::dispatch(callback cb) {
    do {
        stone::Atomic::addFetch(m_total, (uint64_t)1);
        RWMutex::WriteLock lock(m_mutex);
        if(!m_advance) {
...
bool FoxThreadPool::batchDispatch(const std::vector<callback>& cbs) {
    stone::Atomic::addFetch(m_total, cbs.size());
    RWMutex::WriteLock lock(m_mutex);
    if(!m_advance) {
        for(auto cb : cbs) {
...
const std::string& FoxThread::GetFoxThreadName() {
    ...
    uint64_t tid = stone::GetThreadId();
    do {
...
void FoxThread::setThis() {
    m_name = m_name + "_" + std::to_string(stone::GetThreadId());
    s_thread = this;

    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names[stone::GetThreadId()] = m_name;
}

void FoxThread::unsetThis() {
    s_thread = nullptr;
    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names.erase(stone::GetThreadId());
}
...
void FoxThreadManager::init() {
    ...
    for(auto i : m) {
        auto num = stone::GetParamValue(i.second, "num", 0);
        auto name = i.first;
        auto advance = stone::GetParamValue(i.second, "advance", 0);
        if(num <= 0) {
            STONE_LOG_ERROR(g_logger) << "thread pool:" << name
                        << " num:" << num
                        << " advance:" << advance
                        << " invalid";
            continue;
        ...
        if(num == 1) {
            ...
            STONE_LOG_INFO(g_logger) << "init thread : " << i.first;
        } else {
            m_threads[i.first] = FoxThreadPool::ptr(new FoxThreadPool(
                            num, name, advance));
            STONE_LOG_INFO(g_logger) << "init thread pool:" << name
                       << " num:" << num
                       << " advance:" << advance;
        }
    }
...
void FoxThreadManager::start() {
    for(auto i : m_threads) {
        STONE_LOG_INFO(g_logger) << "thread: " << i.first << " start begin";
        i.second->start();
        STONE_LOG_INFO(g_logger) << "thread: " << i.first << " start end";
    }
}

void FoxThreadManager::stop() {
    for(auto i : m_threads) {
        STONE_LOG_INFO(g_logger) << "thread: " << i.first << " stop begin";
        i.second->stop();
        STONE_LOG_INFO(g_logger) << "thread: " << i.first << " stop end";
    }
    for(auto i : m_threads) {
        STONE_LOG_INFO(g_logger) << "thread: " << i.first << " join begin";
        i.second->join();
        STONE_LOG_INFO(g_logger) << "thread: " << i.first << " join end";
    }
}
