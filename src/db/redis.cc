...
static stone::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_redis =
    stone::Config::Lookup("redis.config", std::map<std::string, std::map<std::string, std::string> >(), "redis config");
...
Redis::Redis(const std::map<std::string, std::string>& conf) {
    ...
    m_port = stone::TypeUtil::Atoi(tmp.substr(pos + 1));
    m_passwd = get_value(conf, "passwd");
    m_logEnable = stone::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));

    tmp = get_value(conf, "timeout_com");
    if(tmp.empty()) {
        tmp = get_value(conf, "timeout");
    }
    uint64_t v = stone::TypeUtil::Atoi(tmp);
...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if(c) {
        ...
        if(!m_passwd.empty()) {
            ...
            if(!r) {
                STONE_LOG_ERROR(g_logger) << "auth error:("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if(r->type != REDIS_REPLY_STATUS) {
                STONE_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if(!r->str) {
                STONE_LOG_ERROR(g_logger) << "auth reply str error: NULL("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if(strcmp(r->str, "OK") == 0) {
                ...
                STONE_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
        }
...
ReplyPtr Redis::cmd(const char* fmt, va_list ap) {
    ...
    if(!r) {
        if(m_logEnable) {
            STONE_LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
...
ReplyPtr Redis::cmd(const std::vector<std::string>& argv) {
    ...
    if(!r) {
        if(m_logEnable) {
            STONE_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
    ReplyPtr rt(r, freeReplyObject);
...
ReplyPtr Redis::getReply() {
    ...
    if(m_logEnable) {
        STONE_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
    }
    return nullptr;
}
...
FoxRedis::FoxRedis(stone::FoxThread* thr, const std::map<std::string, std::string>& conf)
    :m_thread(thr)
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if(strcmp(r->str, "OK") == 0) {
        STONE_LOG_INFO(g_logger) << "auth ok: " << r->str << "("
            << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
    } else {
        STONE_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
            << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if(!status) {
        STONE_LOG_INFO(g_logger) << "FoxRedis::ConnectCb "
        ...
        if(!ar->m_passwd.empty()) {
            ...
            if(rt) {
                STONE_LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
            }
        }

    } else {
        STONE_LOG_ERROR(g_logger) << "FoxRedis::ConnectCb "
                    << c->c.tcp.host << ":" << c->c.tcp.port
                    << " fail, error:" << c->errstr;
...
void FoxRedis::DisconnectCb(const redisAsyncContext* c, int status) {
    STONE_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb "
               << c->c.tcp.host << ":" << c->c.tcp.port
               << " status:" << status;
    FoxRedis* ar = static_cast<FoxRedis*>(c->data);
    ar->m_status = UNCONNECTED;
...
void FoxRedis::CmdCb(redisAsyncContext *ac, void *r, void *privdata) {
    ...
    if(ctx->timeout) {
        delete ctx;
        //if(ctx && ctx->fiber) {
        //    STONE_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "' timeout("
    ...
    if(ac->err) {
        if(m_logEnable) {
            stone::replace(ctx->cmd, "\r\n", "\\r\\n");
            STONE_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd
                        << "' "
                        << "(" << ac->err << ") " << ac->errstr;
        }
        if(ctx->fctx->fiber) {
...
//    STONE_LOG_INFO(g_logger) << "DelayTimeCb";
...
bool FoxRedis::init() {
    if(m_thread == stone::FoxThread::GetThis()) {
        return pinit();
    } else {
...
bool FoxRedis::pinit() {
    //STONE_LOG_INFO(g_logger) << "pinit m_status=" << m_status;
    if(m_status != UNCONNECTED) {
        return true;
    }
    auto ctx = redisAsyncConnect(m_host.c_str(), m_port);
    if(!ctx) {
        STONE_LOG_ERROR(g_logger) << "redisAsyncConnect (" << m_host << ":" << m_port
                    << ") null";
        return false;
    }
    if(ctx->err) {
        STONE_LOG_ERROR(g_logger) << "Error:(" << ctx->err << ")" << ctx->errstr;
        return false;
    }
    ctx->data = this;
    ...
    m_context.reset(ctx, stone::nop<redisAsyncContext>);
...
ReplyPtr FoxRedis::cmd(const char* fmt, va_list ap) {
    ...
    if(len == -1) {
        STONE_LOG_ERROR(g_logger) << "redis fmt error: " << fmt;
        return nullptr;
    }
    ...
    //    free(buf);
    //}
    //ctx->scheduler = stone::Scheduler::GetThis();
    //ctx->fiber = stone::Fiber::GetThis();
    //ctx->thread = m_thread;

    FCtx fctx;
    fctx.cmd.append(buf, len);
    free(buf);
    fctx.scheduler = stone::Scheduler::GetThis();
    fctx.fiber = stone::Fiber::GetThis();
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string>& argv) {
    ...
    do {
        ...
        if(len == -1 || !buf) {
            STONE_LOG_ERROR(g_logger) << "redis fmt error";
            return nullptr;
        }
        fctx.cmd.append(buf, len);
        free(buf);
...
void FoxRedis::pcmd(FCtx* fctx) {
    if(m_status == UNCONNECTED) {
        STONE_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port << ") unconnected "
                   << fctx->cmd;
        init();
        if(fctx->fiber) {
...
FoxRedis::Ctx::Ctx(FoxRedis* r)
    ...
    //,scheduler(nullptr)
    ,thread(nullptr) {
    stone::Atomic::addFetch(rds->m_ctxCount, 1);
}

FoxRedis::Ctx::~Ctx() {
    //cancelEvent();
    STONE_ASSERT(thread == stone::FoxThread::GetThis());
    //STONE_ASSERT(destory == 0);
    stone::Atomic::subFetch(rds->m_ctxCount, 1);
    //++destory;
    //cancelEvent();
    if(ev) {
        evtimer_del(ev);
...
void FoxRedis::Ctx::cancelEvent() {
    //if(ev) {
    //    if(thread == stone::FoxThread::GetThis()) {
    //        evtimer_del(ev);
    //        event_free(ev);
    //    } else {
    //        auto e = ev;
...
void FoxRedis::Ctx::EventCb(int fd, short event, void* d) {
    ...
    if(ctx->rds->m_logEnable) {
        stone::replace(ctx->cmd, "\r\n", "\\r\\n");
        STONE_LOG_INFO(g_logger) << "redis cmd: '" << ctx->cmd << "' reach timeout "
                   << (ctx->rds->m_cmdTimeout.tv_sec * 1000
...
FoxRedisCluster::FoxRedisCluster(stone::FoxThread* thr, const std::map<std::string, std::string>& conf)
    :m_thread(thr)
    ,m_status(UNCONNECTED)
    ,m_event(nullptr) {
    m_ctxCount = 0;
...
void FoxRedisCluster::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if(!status) {
        STONE_LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb "
                   << c->c.tcp.host << ":" << c->c.tcp.port
                   << " success";
        if(!ar->m_passwd.empty()) {
            int rt = redisClusterAsyncCommand(ar->m_context.get(), FoxRedisCluster::OnAuthCb, ar, "auth %s", ar->m_passwd.c_str());
            if(rt) {
                STONE_LOG_ERROR(g_logger) << "FoxRedisCluster Auth fail: " << rt;
            }
        }
    } else {
        STONE_LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb "
                    << c->c.tcp.host << ":" << c->c.tcp.port
                    << " fail, error:" << c->errstr;
    }
}
...
void FoxRedisCluster::DisconnectCb(const redisAsyncContext* c, int status) {
    STONE_LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb "
               << c->c.tcp.host << ":" << c->c.tcp.port
               << " status:" << status;
}
...
bool FoxRedisCluster::pinit() {
    ...
    STONE_LOG_INFO(g_logger) << "FoxRedisCluster pinit:" << m_host;
    ...
    if(!ctx) {
        STONE_LOG_ERROR(g_logger) << "redisClusterAsyncConnect (" << m_host
                    << ") null";
        return false;
    }
    if(ctx->err) {
    ...
    m_status = CONNECTED;
    //m_context.reset(ctx, redisAsyncFree);
    m_context.reset(ctx, stone::nop<redisClusterAsyncContext>);
...
void FoxRedisCluster::pcmd(FCtx* fctx) {
    if(m_status != CONNECTED) {
        STONE_LOG_INFO(g_logger) << "redis (" << m_host << ") unconnected "
                   << fctx->cmd;
        init();
        if(fctx->fiber) {
            fctx->scheduler->schedule(&fctx->fiber);
...
void FoxRedisCluster::Ctx::cancelEvent() {
    //STONE_LOG_INFO(g_logger) << "cancelEvent " << stone::FoxThread::GetThis()
    //           << " - " << thread
    //           << " - " << stone::IOManager::GetThis()
    //           << " - " << cancel_count;
    //if(thread != stone::FoxThread::GetThis()) {
    //    STONE_LOG_INFO(g_logger) << "cancelEvent " << stone::FoxThread::GetThis()
    //               << " - " << thread
    ...
    //    //STONE_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << " " << thread->getId()
    //    //           << " this=" << stone::FoxThread::GetThis();
    //    //STONE_ASSERT(thread == stone::FoxThread::GetThis());
    //}
    //STONE_ASSERT(!stone::IOManager::GetThis());
    ////if(stone::Atomic::addFetch(cancel_count) > 1) {
    ////    return;
    ////}
    ////STONE_ASSERT(!stone::Fiber::GetThis());
    ////stone::RWMutex::WriteLock lock(mutex);
    //if(++cancel_count > 1) {
    //    return;
    //}
    //if(ev) {
...
IRedis::ptr RedisManager::get(const std::string& name) {
    ...
    if(r->getType() == IRedis::FOX_REDIS
        ...
        return std::shared_ptr<IRedis>(r, stone::nop<IRedis>);
    }
    lock.unlock();
...
void RedisManager::init() {
    ...
    for(auto& i : m_config) {
        ...
        auto pool = stone::TypeUtil::Atoi(get_value(i.second, "pool"));
        auto passwd = get_value(i.second, "passwd");
        total += pool;
        for(int n = 0; n < pool; ++n) {
            if(type == "redis") {
                stone::Redis* rds(new stone::Redis(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                stone::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                stone::Atomic::addFetch(done, 1);
            } else if(type == "redis_cluster") {
                stone::RedisCluster* rds(new stone::RedisCluster(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                stone::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ...
                auto conf = i.second;
                auto name = i.first;
                stone::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done](){
                    stone::FoxRedis* rds(new stone::FoxRedis(stone::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
                ...
                auto name = i.first;
                stone::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done](){
                    stone::FoxRedisCluster* rds(new stone::FoxRedisCluster(stone::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
