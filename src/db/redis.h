#ifndef __STONE_DB_REDIS_H__
#define __STONE_DB_REDIS_H__
...
public:
    ...
    FoxRedis(stone::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
private:
    struct FCtx {
        std::string cmd;
        stone::Scheduler* scheduler;
        stone::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
public:
    ...
    FoxRedisCluster(stone::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef stone::Singleton<RedisManager> RedisMgr;
