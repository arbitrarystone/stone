...
void on_request_method(void *data, const char *at, size_t length){
    ...
    if(m == HttpMethod::INVALID_METHOD){
        STONE_LOG_WARNING(g_logger)<<"invalid http request method: "
...
void on_request_version(void *data, const char *at, size_t length){
    ...
    if(strncmp(at,"HTTP/1.1",length) == 0){
        ...
        STONE_LOG_WARNING(g_logger)<<"invalid http version: "
            <<std::string(at,length);
...
void on_request_http_field(void *data, const char *field, size_t flen, const char *value, size_t vlen){
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data); 
    if(flen == 0){
        STONE_LOG_WARNING(g_logger)<<"invalid http request field length = 0";
...
void on_response_version(void *data, const char *at, size_t length){
    ...
    if(strncmp(at,"HTTP/1.1",length) == 0){
        ...
        STONE_LOG_WARNING(g_logger)<<"invalid http response version: "
            <<std::string(at,length);
        parser->setError(1001);
        return;
    }
...
void on_response_http_field(void *data, const char *field, size_t flen, const char *value, size_t vlen){
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data); 
    if(flen == 0){
        STONE_LOG_WARNING(g_logger)<<"invalid http response field length = 0";
        parser->setError(1002);
        return;
    }
    parser->getData()->setHeader(std::string(field,flen)
