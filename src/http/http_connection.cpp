...
HttpConnection::~HttpConnection() {
    STONE_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            STONE_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
    ...
    if(!body.empty()) {
        ...
        STONE_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
            << " size=" << body.size();
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        STONE_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = stone::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            STONE_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        ...
        Socket::ptr sock =Socket::CreateTCP(addr);
        if(!sock) {
            STONE_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            STONE_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= stone::GetCurrentMS())
