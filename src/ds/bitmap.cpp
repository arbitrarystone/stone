...
void Bitmap::writeTo(stone::ByteArray::ptr ba) const {
...
bool Bitmap::readFrom(stone::ByteArray::ptr ba) {
    try {
...
Bitmap::ptr Bitmap::compress() const{
    ...
    for(uint32_t i = 0; i < m_dataSize; ++i) {
        ...
        if((cur_count / VALUE_SIZE) > 1) {
            STONE_ASSERT(cur_count % VALUE_SIZE == 0);
            //STONE_ASSERT(cur_count < (1ul << VALUE_SIZE));
            while(cur_count) {
...
void Bitmap::set(uint32_t from, uint32_t size, bool v) {
    ...
    STONE_ASSERT(pos_from == 0);
    STONE_ASSERT(pos_to == 0);
