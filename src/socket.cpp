...
Socket::ptr Socket::CreateTCP(stone::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(stone::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        STONE_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        STONE_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
            << " level=" << level << " option=" << option
            << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept() {
    ...
    if(newsock == -1) {
        STONE_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if(!isValid()) {
        newSock();
        if(STONE_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if(STONE_UNLIKELY(addr->getFamily() != m_family)) {
        STONE_LOG_ERROR(g_logger) << "bind sock.family("
    ...
    if(uaddr) {
        ...
        if(sock->connect(uaddr)) {
            ...
            stone::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        STONE_LOG_ERROR(g_logger) << "bind error errrno=" << errno
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if(!m_remoteAddress) {
        STONE_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(STONE_UNLIKELY(addr->getFamily() != m_family)) {
        STONE_LOG_ERROR(g_logger) << "connect sock.family("
            << m_family << ") addr.family(" << addr->getFamily()
            << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            STONE_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if(!isValid()) {
        STONE_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)) {
        STONE_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if(getpeername(m_sock, result->getAddr(), &addrlen)) {
        //STONE_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
        //    << " errno=" << errno << " errstr=" << strerror(errno);
...
Address::ptr Socket::getLocalAddress() {
    ...
    if(getsockname(m_sock, result->getAddr(), &addrlen)) {
        STONE_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if(m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, stone::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, stone::IOManager::WRITE);
}
...
void Socket::newSock() {
    ...
    if(STONE_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        STONE_LOG_ERROR(g_logger) << "socket(" << m_family
...
//        STONE_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
//            << cert_file << ") error";
//        return false;
//    }
//    if(SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
//        STONE_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
//            << key_file << ") error";
//        return false;
//    }
//    if(SSL_CTX_check_private_key(m_ctx.get()) != 1) {
//        STONE_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
...
//}
//
//SSLSocket::ptr SSLSocket::CreateTCP(stone::Address::ptr address) {
...
//
//int64_t Socket::getSendTimeout(){
//    FdCtx::ptr ctx = stone::FdMgr::GetInstance()->get(m_sock);
//    if(ctx){
//        return ctx->getTimeout(SO_SNDTIMEO);
//    }
//    return -1;
...
//}
//bool Socket::init(int sock){
//    FdCtx::ptr ctx = stone::FdMgr::GetInstance()->get(sock);
//    if(ctx && ctx->isSocket() && !ctx->isClose()){
//        m_sock = sock;
//        m_isConnected = true;
//        initSock();
...
//    if(!isValid()){
//        newSock();
//        if(STONE_UNLICKLY(!isValid())){
//            return false;
//        }
//    }
//
//    if(STONE_UNLICKLY(addr->getFamily() != m_family)){
//        STONE_LOG_ERROR(g_logger)<<"bind sock.family("<<m_family<<")"
...
//
//    if(::bind(m_sock,addr->getAddr(),addr->getAddrLen())){
//        STONE_LOG_ERROR(g_logger)<<"bind errno="<<errno
...
//    }
//    if(STONE_UNLICKLY(addr->getFamily() != m_family)){
//        STONE_LOG_ERROR(g_logger)<<"connect sock.family("<<m_family<<")"
//            <<"addr.family("<<addr->getFamily()<<") not equal"
//            <<",addr ="<<addr->toString();
//        return false;
//    }
...
//        if(::connect_with_timeout(m_sock,addr->getAddr(),addr->getAddrLen(),timeout_ms)){
//            STONE_LOG_ERROR(g_logger)<<"sock ="<<m_sock<<"connect ("<<addr->toString()
//                <<") timeout_ms ="<<timeout_ms<<" error errno="<<errno<<"errstr ="<<strerror(errno);
//            close();
//            return false;
//        }
//    }
